...
export function BookingForm() {
  return (
    <section className="rounded-lg border border-[var(--border)] bg-[var(--bg-card)] p-5 shadow-sm">
      <h2 className="text-xl font-black text-[var(--text-primary)]">Book service or request parts</h2>
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        {['Preferred date', 'Preferred time', 'Service type', 'Unavailable part request'].map((label) => (
      ...
      <Button className="mt-5" variant="accent">Submit request</Button>
    </section>
  )
}
